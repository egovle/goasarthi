...
import { FileText, Clock, CheckCircle, XCircle, User, AlertTriangle, DollarSign } from 'lucide-react';
...
export function AdminDashboard({ user, onLogout }) {
  ...
  const VLES = useMemo(() => getVLEs(), [getVLEs]);
  const allCustomers = useMemo(() => getCustomers(), [getCustomers]);

  const unassignedItems = useMemo(() => tasks
    ...
    .filter(Boolean)
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at)), [tasks, bookings, leads]);
  ...
  }), [bookings, leads, tasks, unassignedItems, allCustomers, complaints, specialRequests]);

  const allBookingsWithDetails = useMemo(() => bookings.map(b => {
  ...
  }).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)), [bookings, tasks, VLES]);

  const allLeadsWithDetails = useMemo(() => leads.map(l => {
  ...
  }).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)), [leads, tasks, VLES]);
